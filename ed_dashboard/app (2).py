...
# Pure-numpy drop-in for sklearn MinMaxScaler — no extra dependency needed.
def _minmax_scale(X):
    """Scale each column of 2-D array X to [0, 1]."""
    X = np.array(X, dtype=float)
    col_min = X.min(axis=0)
    col_max = X.max(axis=0)
    rng = np.where(col_max - col_min == 0, 1e-9, col_max - col_min)
...
def page_mcdm():
    ...
    norm = _minmax_scale(topsis_input)
