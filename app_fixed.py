...
def style_plotly(fig, title=None, height=None):
    ...
    # Clean trace-level labels and hover output.
    fig.for_each_trace(lambda trace: trace.update(name="") if getattr(trace, "name", None) in [None, "undefined", "Undefined"] else None)
    for trace in fig.data:
        if hasattr(trace, "text") and trace.text is not None:
            try:
                trace.text = ["" if pd.isna(x) or str(x).lower() == "undefined" else str(x) for x in trace.text]
            except TypeError:
                if pd.isna(trace.text) or str(trace.text).lower() == "undefined":
                    trace.text = ""
        ...
        if getattr(trace, "hovertemplate", None):
            trace.hovertemplate = str(trace.hovertemplate).replace("undefined", "").replace("Undefined", "")
...
def page_rl():
    ...
    if not policy_summary.empty:
        ...
        fig = go.Figure(data=go.Heatmap(
            ...
            y=[f'Bed={b}' for b in beds],
            colorscale='Blues',
            colorbar=dict(title="Visits", tickfont=dict(color='#000000'), titlefont=dict(color='#000000')),
            text=z.astype(int),
            texttemplate="%{text}",
            textfont=dict(color='#000000', size=11),
